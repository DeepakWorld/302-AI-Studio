...
export class IpcStructureGenerator {
	...
	public generatePreloadServicesModule(structure: GeneratedIpcStructure): string {
		...
		const typeImports =
			customTypes.size > 0
				? `import type { ${Array.from(customTypes).join(", ")} } from '@shared/types.js';`
				: "";
	...
	const registrations = structure.services
		.map((service) => {
			...
			const methodHandlers = service.methods // <-- Renamed variable from 'methods' to 'methodHandlers' to avoid shadowing
				.map((method) => {
			...
			return `\t// ${service.serviceName} service registration\n${methodHandlers}`; // <-- Uses safe local block scope variable
		})
		.join("\n\n");

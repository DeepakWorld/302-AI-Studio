...
export interface AutoGeneratedIpcServices {
	...
	threadStateService: {
		updateBusyState(data: { threadId: string; isBusy: boolean; reason?: string }): Promise<void>;
	...
	tabService: {
		...
		triggerCreateSkillSummary(threadId: string): Promise<boolean>;
	};
	aiApplicationService: {
		...
		handleAiApplicationReloadIpc(tabId: string): Promise<void>;
		refreshAiApplications(): Promise<void>;
	};
	appService: {
		getUserAgentFragment(): Promise<string>;
		getTheme(): Promise<Theme>;
		setTheme(theme: Theme): Promise<void>;
		restartApp(): Promise<void>;
		resetAllData(): Promise<void>;
		clearChatHistory(): Promise<void>;
		extractZipBlob(zipData: ArrayBuffer, originalFileName?: string): Promise<string>;
		...
		zipDirectory(dirPath: string, zipName: string): Promise<ArrayBuffer>;
		createSkillTempDir(skillName: string): Promise<{ rootPath: string; skillMdPath: string }>;
		deleteTempDir(dirPath: string): Promise<void>;
	};
	dataService: {
		...
		selectFolderForUpload(): Promise<{ folderPath: string; folderName: string } | null>;
		exportChatToFile(
			content: string,
			extension: string,
			filterName: string,
	...
	localVibeService: {
		copyToWorkspaceByIpc(
			sourcePath: string,
			containerPath: string,
		): Promise<{ success: boolean; error?: string }>;
		getComposeDirectory(): Promise<string>;
		openComposeDirectory(): Promise<boolean>;
		openWorkspaceDirectory(subPath: string): Promise<boolean>;
		deleteWorkspaceDirectory(subPath: string): Promise<{ success: boolean; error?: string }>;
		renameWorkspaceDirectory(
			oldSubPath: string,
			newSubPath: string,
		): Promise<{ success: boolean; error?: string }>;
		listWorkspaceDirectories(): Promise<string[]>;
		getLocalBaseUrl(): Promise<string | null>;
		getSandboxStatus(): Promise<{
			isRunning: boolean;
			isOperating: boolean;
		}>;
		triggerSystemRestart(): Promise<{ isOk: boolean }>;
		validPodman(): Promise<{
			isOk: boolean;
			isValid: boolean;
			output?: string;
		...
		startPodmanHealthCheck(): Promise<{ isOk: boolean }>;
		installWSL(): Promise<{ isOk: boolean }>;
		installScoop(): Promise<{ isOk: boolean }>;
		installHomebrew(): Promise<{ isOk: boolean }>;
		installPodman(): Promise<{ isOk: boolean }>;
		initPodmanMachine(): Promise<{ isOk: boolean }>;
		stopLocalSandboxByIpc(): Promise<{ isOk: boolean; output?: string; error?: string }>;
		ensureLocalSandboxRunning(): Promise<{
			isOk: boolean;
			port?: number;
			error?: string;
			wasAlreadyRunning: boolean;
		}>;
		startPodmanMachine(): Promise<{
			isOk: boolean;
...
export const autoGeneratedServices: AutoGeneratedIpcServices = {
	...
	codeAgentService: {
		...
		updateClaudeCodeSandboxThinkingBudget: (sandbox_id: string, maxThinkingToken: number) =>
			ipcRenderer.invoke(
		...
		addClaudeCodeSandboxMCP: (
			...
			MCPInfos: { url: string; name: string }[],
			mode: string,
		) => ipcRenderer.invoke("codeAgentService:addClaudeCodeSandboxMCP", sandboxId, MCPInfos, mode),
		createThreadForSession: (
			threadId: string,
			sandboxId: string,
			sessionId: string,
		...
		getThreadIdBySessionId: (sandboxId: string, sessionId: string) =>
			ipcRenderer.invoke("codeAgentService:getThreadIdBySessionId", sandboxId, sessionId),
		setIsManualNoteBySession: (sandboxId: string, sessionId: string, isManualNote: boolean) =>
			ipcRenderer.invoke(
	...
	windowService: {
		...
		handleNavigateToUrl: (title: string, type: TabType, href: string) =>
			ipcRenderer.invoke("windowService:handleNavigateToUrl", title, type, href),
		...
		navigateToThread: (threadId: string, sourceWindowId?: string) =>
			ipcRenderer.invoke("windowService:navigateToThread", threadId, sourceWindowId),
	},
	deepLinkService: {
		simulateDeepLink: (url: string) => ipcRenderer.invoke("deepLinkService:simulateDeepLink", url),
	},
	threadStateService: {
		updateBusyState: (data: { threadId: string; isBusy: boolean; reason?: string }) =>
			ipcRenderer.invoke("threadStateService:updateBusyState", data),
		getBusyThreads: () => ipcRenderer.invoke("threadStateService:getBusyThreads"),
	},
	tabService: {
		handleNewTabWithThread: (
			threadId: string,
		...
		triggerCreateSkillSummary: (threadId: string) =>
			ipcRenderer.invoke("tabService:triggerCreateSkillSummary", threadId),
	},
	aiApplicationService: {
		getAiApplicationUrl: (applicationId: number) =>
			ipcRenderer.invoke("aiApplicationService:getAiApplicationUrl", applicationId),
		handleAiApplicationReloadIpc: (tabId: string) =>
			ipcRenderer.invoke("aiApplicationService:handleAiApplicationReloadIpc", tabId),
		refreshAiApplications: () => ipcRenderer.invoke("aiApplicationService:refreshAiApplications"),
	},
	appService: {
		getUserAgentFragment: () => ipcRenderer.invoke("appService:getUserAgentFragment"),
		...
		extractZipBlob: (zipData: ArrayBuffer, originalFileName?: string) =>
			ipcRenderer.invoke("appService:extractZipBlob", zipData, originalFileName),
		scanDirectory: (dirPath: string) => ipcRenderer.invoke("appService:scanDirectory", dirPath),
		readFile: (filePath: string) => ipcRenderer.invoke("appService:readFile", filePath),
		readFileAsBuffer: (filePath: string) =>
			ipcRenderer.invoke("appService:readFileAsBuffer", filePath),
		writeFile: (filePath: string, content: string) =>
			ipcRenderer.invoke("appService:writeFile", filePath, content),
		createDirectory: (dirPath: string) => ipcRenderer.invoke("appService:createDirectory", dirPath),
		deleteFile: (filePath: string) => ipcRenderer.invoke("appService:deleteFile", filePath),
		deleteDirectory: (dirPath: string) => ipcRenderer.invoke("appService:deleteDirectory", dirPath),
		renameFile: (oldPath: string, newPath: string) =>
			ipcRenderer.invoke("appService:renameFile", oldPath, newPath),
		zipDirectory: (dirPath: string, zipName: string) =>
			ipcRenderer.invoke("appService:zipDirectory", dirPath, zipName),
		createSkillTempDir: (skillName: string) =>
			ipcRenderer.invoke("appService:createSkillTempDir", skillName),
		deleteTempDir: (dirPath: string) => ipcRenderer.invoke("appService:deleteTempDir", dirPath),
	},
	dataService: {
		...
		selectFolderForUpload: () => ipcRenderer.invoke("dataService:selectFolderForUpload"),
		exportChatToFile: (
			content: string,
			extension: string,
			filterName: string,
	...
	localVibeService: {
		copyToWorkspaceByIpc: (sourcePath: string, containerPath: string) =>
			ipcRenderer.invoke("localVibeService:copyToWorkspaceByIpc", sourcePath, containerPath),
		getComposeDirectory: () => ipcRenderer.invoke("localVibeService:getComposeDirectory"),
		openComposeDirectory: () => ipcRenderer.invoke("localVibeService:openComposeDirectory"),
		openWorkspaceDirectory: (subPath: string) =>
			ipcRenderer.invoke("localVibeService:openWorkspaceDirectory", subPath),
		deleteWorkspaceDirectory: (subPath: string) =>
			ipcRenderer.invoke("localVibeService:deleteWorkspaceDirectory", subPath),
		renameWorkspaceDirectory: (oldSubPath: string, newSubPath: string) =>
			ipcRenderer.invoke("localVibeService:renameWorkspaceDirectory", oldSubPath, newSubPath),
		listWorkspaceDirectories: () => ipcRenderer.invoke("localVibeService:listWorkspaceDirectories"),
		getLocalBaseUrl: () => ipcRenderer.invoke("localVibeService:getLocalBaseUrl"),
		getSandboxStatus: () => ipcRenderer.invoke("localVibeService:getSandboxStatus"),
		triggerSystemRestart: () => ipcRenderer.invoke("localVibeService:triggerSystemRestart"),
		validPodman: () => ipcRenderer.invoke("localVibeService:validPodman"),
		startPodmanHealthCheck: () => ipcRenderer.invoke("localVibeService:startPodmanHealthCheck"),
		installWSL: () => ipcRenderer.invoke("localVibeService:installWSL"),
		installScoop: () => ipcRenderer.invoke("localVibeService:installScoop"),
		installHomebrew: () => ipcRenderer.invoke("localVibeService:installHomebrew"),
		installPodman: () => ipcRenderer.invoke("localVibeService:installPodman"),
		initPodmanMachine: () => ipcRenderer.invoke("localVibeService:initPodmanMachine"),
		stopLocalSandboxByIpc: () => ipcRenderer.invoke("localVibeService:stopLocalSandboxByIpc"),
		ensureLocalSandboxRunning: () =>
			ipcRenderer.invoke("localVibeService:ensureLocalSandboxRunning"),
		startPodmanMachine: () => ipcRenderer.invoke("localVibeService:startPodmanMachine"),
	},
	mcpService: {
	...
	notificationService: {
		notifyTaskCompleted: (options: {
			...
			windowId: string;
			tabId: string;
		}) => ipcRenderer.invoke("notificationService:notifyTaskCompleted", options),
		requestPermission: () => ipcRenderer.invoke("notificationService:requestPermission"),
	},
	providerService: {
		handle302AIProviderChange: (apiKey: string) =>
			ipcRenderer.invoke("providerService:handle302AIProviderChange", apiKey),
		get302AIApiKey: () => ipcRenderer.invoke("providerService:get302AIApiKey"),
	},
	threadService: {
...
export interface ElectronAPIExtension {
	...
	deepLinkService: AutoGeneratedIpcServices["deepLinkService"];
	threadStateService: AutoGeneratedIpcServices["threadStateService"];
	...
	localVibeService: AutoGeneratedIpcServices["localVibeService"];
	mcpService: AutoGeneratedIpcServices["mcpService"];
	notificationService: AutoGeneratedIpcServices["notificationService"];
